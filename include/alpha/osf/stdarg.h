...
#define va_start(list, start)                                                                   \
    ((void)((list)._a0 =                                                                        \
                (__typecode(__firstarg) == 1 ? (char *)&__firstarg + 48 : (char *)&__firstarg), \
            (list)._offset =                                                                    \
                (__typecode(start) == 1 ? (char *)&start + 56 : (char *)&start + 8) - (list)._a0))
#define va_arg(list, mode)                                                            \
    (*(mode *)((list)._offset += (int)((sizeof(mode) + 7) & ~7),                      \
               (__typecode(mode) == 1 && sizeof(mode) == 4)                           \
               ? ((list)._tmp = (float)*(double *)((list)._a0 + (list)._offset -      \
                                                   ((list)._offset <= 48 ? 56 : 8))), \
               (char *)&(list)._tmp                                                   \
