...
static Tree root1(Tree p)
{
    ...
    case CVF:
        ...
        return root1(p->kids[0]);
    case CVI:
        if ((optype(p->op) == U || optype(p->op) == I) && p->type->size < p->kids[0]->type->size &&
...
char *opname(int op)
{
    static char *opnames[] = { "",      "CNST",  "ARG",   "ASGN", "INDIR", "CVC",   "CVD",    "CVF",
                               "CVI",   "CVP",   "CVS",   "CVU",  "NEG",   "CALL",  "*LOAD*", "RET",
                               "ADDRG", "ADDRF", "ADDRL", "ADD",  "SUB",   "LSH",   "MOD",    "RSH",
                               "BAND",  "BCOM",  "BOR",   "BXOR", "DIV",   "MUL",   "EQ",     "GE",
                               "GT",    "LE",    "LT",    "NE",   "JUMP",  "LABEL", "AND",    "NOT",
                               "OR",    "COND",  "RIGHT", "FIELD" },
