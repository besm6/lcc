...
static int emittype(Type ty, int lev, int col)
{
    ...
    case UNSIGNED:
        if (ty == chartype) /* plain char is a subrange of itself */
            ...
            print("=r1;0;%U;", ty->u.sym->u.limits.max.i), col += 4 + 2.408 * ty->size + 1;
        break;
    ...
    case UNION: {
        ...
        for (p = fieldlist(ty); p; p = p->link) {
            ...
            if (p->lsb)
                print(",%d,%d;", 8 * p->offset + (IR->little_endian ? fieldright(p) : fieldleft(p)),
                      fieldsize(p));
            else
...
void stabblock(int brace, int lev, Symbol *p)
{
    ...
    if (IR == &sparcIR)
        ...
        int lab = genlabel(1);
        print(".stabn 0x%x,0,%d,%s%d-%s\n", brace == '{' ? N_LBRAC : N_RBRAC, lev, stabprefix, lab,
              cfunc->x.name);
        print("%s%d:\n", stabprefix, lab);
    }
}
    ...
    if ((p->sclass == AUTO && p->scope == GLOBAL) ||
        ...
        if (p->x.regnode) {
            ...
            if (p->x.regnode->set == FREG)
                r += 32; /* floating point */
            print(".stabs \"%s:%c%d\",%d,0,", p->name, p->scope == PARAM ? 'P' : 'r', tc, N_RSYM);
            print("%d,%d\n", sz, r);
        }
        return;
