...
static Node replace(Node p)
{
    if (p && (generic(p->op) == INDIR && generic(p->kids[0]->op) == ADDRL &&
        ...
        if (generic(p->op) == INDIR && isaddrop(p->kids[0]->op))
            p = newnode(p->op, newnode(p->kids[0]->op, NULL, NULL, p->kids[0]->syms[0]), NULL,
                        NULL);
...
static Node prune(Node forest)
{
    ...
    for (p = forest; p; p = p->link) {
        ...
        if ((generic(p->op) == ASGN && generic(p->kids[0]->op) == ADDRL &&
             p->kids[0]->syms[0]->temporary && p->kids[0]->syms[0]->u.t.cse == p->kids[1])) {
            ...
            if ((generic(p->kids[1]->op) == INDIR && isaddrop(p->kids[1]->kids[0]->op) &&
                ...
                ((generic(p->kids[1]->op) == INDIR && isaddrop(p->kids[1]->kids[0]->op)) &&
                 tmp->sclass == AUTO) ||
...
static Node tmpnode(Node p)
{
    ...
    if (--p->count == 0)
        p->syms[2] = NULL;
    p = newnode(INDIR + ttob(tmp->type), newnode(ADDRL + ttob(voidptype), NULL, NULL, tmp), NULL,
                NULL);
    p->count = 1;
    return p;
}
