...
main()
{
    ...
    TstCond(Failure,
            (Zero + Zero == Zero) && (One - One == Zero) && (One > Zero) && (One + One == Two),
...
*/
    ...
    if ((X == Zero) && (Y == Zero) && (Z == Zero) && (T <= Zero)) {
        ...
        Y1        = Y1 - Half;
        if ((X == Zero) && (Y == Zero) && (Z == Zero) && (T == Zero) && (StickyBit == Zero) &&
            (Y1 == Half)) {
            RMult = Rounded;
            printf("Multiplication appears to round correctly.\n");
        } else if ((X + U2 == Zero) && (Y < Zero) && (Z + U2 == Zero) && (T < Zero) &&
                   (StickyBit + U2 == Zero) && (Y1 < Half)) {
            RMult = Chopped;
    ...
    if (!((X > Zero) || (Y > Zero) || (Z > Zero) || (T > Zero))) {
        ...
        Y2 = Y1 - Y2;
        Y1 = (F9 - U1) / F9;
        if ((X == Zero) && (Y == Zero) && (Z == Zero) && (T == Zero) && (Y2 == Zero) &&
...
*/
    ...
    StickyBit = Zero;
    if ((GMult == Yes) && (GDiv == Yes) && (GAddSub == Yes) && (RMult == Rounded) &&
        (RDiv == Rounded) && (RAddSub == Rounded) && (FLOOR(RadixD2) == RadixD2)) {
    ...
    TstCond(Flaw,
            !(GMult == No || GDiv == No || GAddSub == No || RMult == Other || RDiv == Other ||
              RAddSub == Other),
            "lack(s) of guard digits or failure(s) to correctly round or chop\n(noted above) count "
            "as one flaw in the final tally below");
    /*=============================================*/
    Milestone = 60;
    /*=============================================*/
    printf("\n");
...
overflow:
    ...
    for (Indx = 1; Indx <= 3; ++Indx) {
        ...
        if (Z != Zero) {
            V9 = SQRT(Z);
            Y  = V9 * V9;
            if (Y / (One - Radix * E9) < Z || Y > (One + Radix * E9) * Z) { /* dgh: + E9 --> * E9 */
                if (V9 > U1)
                    BadCond(Serious, "");
                else
                    BadCond(Defect, "");
...
*/
    ...
    printf("\n");
    {
        static char *msg[] = { "FAILUREs  encountered =", "SERIOUS DEFECTs  discovered =",
    ...
    if ((ErrCnt[Failure] + ErrCnt[Serious] + ErrCnt[Defect] + ErrCnt[Flaw]) > 0) {
        ...
        if (!((RMult == Rounded) && (RDiv == Rounded) && (RAddSub == Rounded) &&
            ...
            if (StickyBit >= One && (Radix - Two) * (Radix - Nine - One) == Zero) {
                ...
                if ((Radix == Two) && ((Precision - Four * Three * Two) *
...
double pow(x, y) /* return x ^ y (exponentiation) */
double x, y;
{
